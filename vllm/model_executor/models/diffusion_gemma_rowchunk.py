...
def _uniform_from_mantissa(
    mantissa: torch.Tensor,
    *,
    fast_fp32: bool,
) -> torch.Tensor:
    if fast_fp32:
        return (mantissa.to(torch.float32) + 0.5) * _FLOAT_2_NEG_53
...
def stable_uniform_from_indices(
    ...
    By default this preserves the original int64 hash / 53-bit mantissa /
    fp64-scale / fp32 stream exactly. ``VLLM_DIFFUSION_GEMMA_ROW_CHUNK_FAST_FP32_RNG``
    skips the fp64 conversion for speed, but it is a statistical compatibility
    mode rather than a bit-exact stream: rare fp32 midpoint mantissas can differ.
    """
    x = (
    ...
    return _uniform_from_mantissa(
        mantissa,
        fast_fp32=envs.VLLM_DIFFUSION_GEMMA_ROW_CHUNK_FAST_FP32_RNG,
    )
...
def diffusion_gemma_softcap_row_chunked_sample_soft_embeds(
    ...
    *,
    ...
    """Exact row-chunked materialized DiffusionGemma sampler.
    ...
    ``[row_chunk_size, vocab]`` so high-concurrency decode can avoid OOM.
