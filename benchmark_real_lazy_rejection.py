...
def run_bench():
    ...
    output_token_ids = torch.zeros((batch_size, max_spec_len + 1), dtype=torch.int32, device=device)
    ...
    for _ in range(10):
        ...
        old_rejection_random_sample_kernel[(batch_size,)](
            output_token_ids, cu_num_draft_tokens, draft_token_ids, target_probs, recovered_token_ids, uniform_probs_reject_0, max_spec_len, vocab_size
        )
        new_rejection_random_sample_kernel[(batch_size,)](
            output_token_ids, cu_num_draft_tokens, draft_token_ids, target_probs, inv_q, uniform_probs_reject_0, max_spec_len, vocab_size, BLOCK_SIZE=8192
        )
    ...
    for _ in range(1000):
        new_rejection_random_sample_kernel[(batch_size,)](
            output_token_ids, cu_num_draft_tokens, draft_token_ids, target_probs, inv_q, uniform_probs_reject_4, max_spec_len, vocab_size, BLOCK_SIZE=8192
        )
    torch.cuda.synchronize()
    end = time.perf_counter()
    ...
    uniform_probs_accept_all = torch.zeros(num_tokens, dtype=torch.float32, device=device)
    start = time.perf_counter()
    for _ in range(1000):
        new_rejection_random_sample_kernel[(batch_size,)](
            output_token_ids, cu_num_draft_tokens, draft_token_ids, target_probs, inv_q, uniform_probs_accept_all, max_spec_len, vocab_size, BLOCK_SIZE=8192
        )
    torch.cuda.synchronize()
    end = time.perf_counter()
    ...
    print(f"New Lazy Path (All accepted): {new_time_accept_all:.4f} ms ({old_time/new_time_accept_all:.2f}x speedup)")
