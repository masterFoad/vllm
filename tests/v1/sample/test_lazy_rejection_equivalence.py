...
from benchmark_real_lazy_rejection import (
    new_rejection_random_sample_kernel,
    old_rejection_random_sample_kernel,
...
def test_seeded_equivalence():
    ...
    # Old Path
    output_token_ids_old = torch.zeros(
    ...
    # New Path
    output_token_ids_new = torch.zeros(
        (batch_size, max_spec_len + 1), dtype=torch.int32, device=device
    )

    new_rejection_random_sample_kernel[(batch_size,)](
        output_token_ids_new,
        cu_num_draft_tokens,
        draft_token_ids,
        target_probs,
        inv_q,
    ...
    assert torch.equal(output_token_ids_old, output_token_ids_new), (
        "Lazy rejection sampling output does not match eager output!"
    )
